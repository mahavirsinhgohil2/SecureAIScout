...
| 0:15-0:35 | Work view | Click `Start demo scan`; let the event trace appear. | "The cache preserves 33 events, including investigation and context-aware triage." |
...
| 0:55-1:20 | Verify trace | Scroll to command injection. | "The first patch is rejected because unsafe command execution remains; the agent retries." |
| 1:20-1:40 | Command card | Open its proposed change and proof. | "The second attempt passes syntax, command safety, sandbox rescan, and demo tests." |
| 1:40-2:05 | Review summary | Show risk and metric cards. | "The run reports 6 verified real findings and a computed risk change from 78 to 1." |
